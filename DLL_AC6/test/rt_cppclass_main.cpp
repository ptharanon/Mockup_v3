...
/* Function: customeInitializer ====================================================
*
* Abstract:
*   Custom initialize the model using input arrays
*
*/
static void customeInitializer(MODEL_CLASSNAME & mdl, double speedInput_arr[],double torqueInput_arr[],double timeVector[])
{
#if defined(MULTITASKING)
	int i;
	for (i = 0; i < NUMST; i++) {
	...
	* Initialize the model *
	************************/
	mdl.customeInitializer(speedInput_arr,torqueInput_arr,timeVector);
}
...
*/
/*
static double *csvLoader(string fileName,int size, bool isTime)
{
	...
	while (std::getline(inputFile, buffer_line))
	{
		std::istringstream iss{ buffer_line };
		
		// Read the tokens from current line separated by comma
		std::vector<std::string> tokens; // Tokens vector
		...
		// If the data is being loaded then use token[1]
		if (!isTime)
		{
			// Convert String to double (ignore the first token (time))
			temp_arr[i] = std::stod(tokens[1]);
		}
		else
		{
			// Convert String to double (time vector)
			temp_arr[i] = std::stod(tokens[0]);
		}
		i++;
	}
	return temp_arr;
...
extern "C"
{
	__declspec(dllexport) double *runSimulation(double timeVector[], double speedInput_arr[], double torqueInput_arr[], int size)
	{
		/* External mode */
	...
	************************/
	...
	int size = getsize(speed_fileName);
	double *speedInput_arr = new double[size];
	double *torqueInput_arr = new double[size];
	double *timeVector = new double[size];

	timeVector = csvLoader(speed_fileName,size,true);
	speedInput_arr = csvLoader(speed_fileName,size,false);
	torqueInput_arr = csvLoader(torque_fileName,size,false);
	*/

	/***************************************************
	*	Calculate sampling time and simulation time    *
	***************************************************/
	double sampleTime = timeVector[1] - timeVector[0];
	uint32_T simulationTime = (size - 1)*sampleTime / MODEL_SAMPLETIME;
	...
	/**********************************
	* Customized-initialize the model *
	***********************************/
	double *result_Arr = new double[(int)6 * simulationTime];
	int i = 0;

	customeInitializer(MODEL_INSTANCE, speedInput_arr, torqueInput_arr, timeVector);
	...
	*/
	...
	while (rtmGetErrorStatus(MODEL_INSTANCE.getRTM()) == NULL &&
		...
		myfile << MODEL_INSTANCE.ac6_example_Y.Statorcurrent << ","
			...
			<< MODEL_INSTANCE.ac6_example_Y.ElectromagneticTorque[0] << ","
			<< MODEL_INSTANCE.ac6_example_Y.DCbusvoltage << endl;
		*/
		
		result_Arr[i] = MODEL_INSTANCE.ac6_example_Y.Statorcurrent;
		result_Arr[i+1*simulationTime] = MODEL_INSTANCE.ac6_example_Y.Rotorspeed[0];
		result_Arr[i+2*simulationTime] = MODEL_INSTANCE.ac6_example_Y.Rotorspeed[1];
		result_Arr[i+3*simulationTime] = MODEL_INSTANCE.ac6_example_Y.ElectromagneticTorque[0];
		result_Arr[i+4*simulationTime] = MODEL_INSTANCE.ac6_example_Y.ElectromagneticTorque[1];
		result_Arr[i+5*simulationTime] = MODEL_INSTANCE.ac6_example_Y.DCbusvoltage;

		i++;

		//printf("The time is: %d\n", (MODEL_INSTANCE.getRTM())->Timing.clockTick0);
		
		if ((MODEL_INSTANCE.getRTM())->Timing.clockTick0 >= simulationTime)
		{
			//printf("The time is: %d\n", (MODEL_INSTANCE.getRTM())->Timing.clockTick0);
			break;
		}
